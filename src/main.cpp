...
// update freq global variable to use in LED threads
// we aggragate all magnitudes within the correct frequency range
...
//function for dyskinesia thread
...
int main() {
  ...
  while (1) {
    ...
    for(int i=0;i<FFT_SIZE;i++){
      ...
      // FFT input is the magnitude of the DPS of all axis combined
      fft_input[i] = sqrtf(gyro_x[i] * gyro_x[i] + gyro_y[i] * gyro_y[i] + gyro_z[i] * gyro_z[i]);
    }

    //perform FFT and update freq global variable
